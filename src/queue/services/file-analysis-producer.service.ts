...
export class FileAnalysisProducerService {
  ...
  async sendFileAnalysisRequest(data: FileAnalysisMessage): Promise<string> {
    ...
    try {
      ...
      this.logger.log(
        `📤 Sending file analysis request to queue for file: ${data.fileName}`,
      );
      ...
      try {
        ...
        const messageId =
          batchResults.length > 0 ? batchResults[0].MessageId : undefined;
        ...
        this.logger.warn(
          `⚠️ Unable to record SQS MessageId for Job ${job.jobUuid}: ${e}`,
        );
      }

      this.logger.log(
        `✅ File analysis request sent successfully for file: ${data.fileName} (Job: ${job.jobUuid})`,
      );
      return job.jobUuid;
    } catch (error) {
      const errorMessage =
      ...
      this.logger.error(
        `❌ Failed to send file analysis request for file: ${data.fileName}`,
        error,
      );
      ...
      if (job && job.jobUuid) {
        try {
          ...
          this.logger.log(
            `🚨 Marked job ${job.jobUuid} as FAILED due to queue send error`,
          );
        } catch (updateError) {
          this.logger.error(
            `❌ Failed to update job status to FAILED for job ${job.jobUuid}`,
            updateError,
          );
        }
      }
