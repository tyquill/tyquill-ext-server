...
export class SlackService {
  ...
  constructor(private configService: ConfigService) {
    const webhookUrl = 'https://hooks.slack.com/services/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';
    if (webhookUrl) {
  ...
  async notifyNewUserSignup(userData: {
    ...
    try {
      ...
      this.logger.log(`Slack notification sent for new user: ${userData.email}`);
    } catch (error) {
  ...
  async notifyError(error: Error, context?: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error('Failed to send error notification to Slack:', sendError);
    }
  }
}